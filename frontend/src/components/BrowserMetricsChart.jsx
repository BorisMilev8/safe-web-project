...
export default function BrowserMetricsChart() {
  ...
  const bestBrowser = useMemo(() => {
    if (!summaryData.length) return "N/A";

    const valid = summaryData.filter(
      (row) => row.avg_load_time_sec !== null && row.avg_load_time_sec !== undefined
    );

    if (!valid.length) return "N/A";

    const best = [...valid].sort(
      (a, b) => a.avg_load_time_sec - b.avg_load_time_sec
    )[0];

    return DISPLAY_NAMES[best.browser] || best.browser;
  }, [summaryData]);

  const avgLoadAcrossBrowsers = useMemo(() => {
    const vals = summaryData
      .map((row) => row.avg_load_time_sec)
      .filter((v) => v !== null && v !== undefined);
    if (!vals.length) return "N/A";
    return formatValue(vals.reduce((a, b) => a + b, 0) / vals.length);
  }, [summaryData]);

  const avgCpuAcrossBrowsers = useMemo(() => {
    const vals = summaryData
      .map((row) => row.avg_cpu_percent)
      .filter((v) => v !== null && v !== undefined);
    if (!vals.length) return "N/A";
    return formatValue(vals.reduce((a, b) => a + b, 0) / vals.length);
  }, [summaryData]);

  const avgMemoryAcrossBrowsers = useMemo(() => {
    const vals = summaryData
      .map((row) => row.avg_rss_mb)
      .filter((v) => v !== null && v !== undefined);
    if (!vals.length) return "N/A";
    return formatValue(vals.reduce((a, b) => a + b, 0) / vals.length);
  }, [summaryData]);
  ...
  if (loading) return <div className="card">Loading benchmark data...</div>;
  if (error) return <div className="card error">Error: {error}</div>;
  if (!summaryData.length) return <div className="card">No benchmark data found.</div>;
    ...
    <>
      <section className="summary-cards">
        <div className="card stat-card">
          ...
          <span className="stat-value">{avgLoadAcrossBrowsers} sec</span>
        </div>

        <div className="card stat-card">
          <span className="stat-label">Avg CPU</span>
          <span className="stat-value">{avgCpuAcrossBrowsers} %</span>
        </div>

        <div className="card stat-card">
          <span className="stat-label">Avg Memory</span>
          <span className="stat-value">{avgMemoryAcrossBrowsers} MB</span>
        </div>

        <div className="card stat-card">
          <span className="stat-label">Best Load Time</span>
          <span className="stat-value">{bestBrowser}</span>
        </div>
      </section>
        ...
        <div className="card">
          ...
          <div className="meta-grid">
            ...
            <div className="meta-item">
              ...
              <span className="meta-value">
                {configData?.urls_tested?.join(", ") ?? "N/A"}
              </span>
            </div>
          </div>
        </div>
